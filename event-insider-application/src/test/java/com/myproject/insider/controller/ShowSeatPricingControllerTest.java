...
class ShowSeatPricingControllerTest {
    ...
    void create_withEmptyBody_returns400() throws Exception {
        mockMvc.perform(post("/api/v1/show-seat-pricings").contentType(APPLICATION_JSON).content("{}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.meta.code").value("EVENT_INSIDER_SERVICE_ERROR_400"))
                .andExpect(jsonPath("$.data.errors.showId").exists())
                .andExpect(jsonPath("$.data.errors.eventCategoryId").exists())
                .andExpect(jsonPath("$.data.errors.basePrice").exists())
